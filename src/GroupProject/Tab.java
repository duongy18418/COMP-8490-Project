...
public class Tab extends JFrame {
	...
	private static SoundUtilityJOAL soundJOAL = new SoundUtilityJOAL(); 
	...
	public static BranchGroup createSceneGraph() {
		...
		Appearance boxAppearance = new Appearance();
		ColoringAttributes boxColorAttr = new ColoringAttributes(new Color3f(0.7f, 0.7f, 0.7f),
	...
	private static void addSpeakerGrills(TransformGroup parent) {
		...
		ColoringAttributes backgroundColor = new ColoringAttributes(new Color3f(0.1f, 0.1f, 0.1f),
				ColoringAttributes.NICEST);
		speakerBackgroundApp.setColoringAttributes(backgroundColor);

		float[][] speakerPositions = { { -0.25f, 0.0f }, { 0.25f, 0.0f } };
		float radius = 0.15f;
		float innerRadius = radius * 0.95f;

		for (float[] pos : speakerPositions) {
			Cylinder background = new Cylinder(radius, 0.001f, Cylinder.GENERATE_NORMALS, 60, 1, speakerBackgroundApp);
			...
			ColoringAttributes lineColor = new ColoringAttributes(new Color3f(0.0f, 0.0f, 0.0f),
					ColoringAttributes.NICEST);
			...
			for (float x = -innerRadius; x <= innerRadius; x += spacing) {
				...
				for (float y = -maxY; y <= maxY; y += spacing) {
					...
					line1Transform.setTranslation(new Vector3f(pos[0] + x, pos[1] + y, 0.205f));
					...
					line2Transform.setTranslation(new Vector3f(pos[0] + x, pos[1] + y, 0.205f));
	...
	private static void addHandle(TransformGroup parent) {
		...
		ColoringAttributes handleColor = new ColoringAttributes(new Color3f(0.6f, 0.6f, 0.6f),
				ColoringAttributes.NICEST);
	...
	private static void createButton(TransformGroup parent, TransformGroup buttonTG, Color3f color, String name,
	...
	public static void initial_Sound() {
		for(int i = 0; i<7;i++) {
			
			if (!soundJOAL.load(i+"", 0f, 0f, 10f, true))
				System.out.println("Could not load " + i+".wav");
		}
	}
