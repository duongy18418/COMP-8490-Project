...
public class CommonsKD extends JPanel {
	...
	public static Background solid_Background(Color3f clr) {		
		Background back_Ground = new Background();
		back_Ground.setColor(clr);
		back_Ground.setApplicationBounds(CommonsKD.hundred_BS);
	...
	public CommonsKD(BranchGroup sceneBG) {
		GraphicsConfiguration config = SimpleUniverse.getPreferredConfiguration();
		Canvas3D canvas = new Canvas3D(config);		
		SimpleUniverse su = new SimpleUniverse(canvas);    // create a SimpleUniverse
		
		sceneBG.addChild(solid_Background(Grey));
	...
	public static void main(String[] args) {
		frame = new JFrame("KD's Common File");            // NOTE: change XY to student's initials
		frame.getContentPane().add(new CommonsKD(create_Scene()));  // create an instance of the class
